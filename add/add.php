...
<body>
    ...
    <?php
        ...
        $query = "INSERT INTO private( Username, Password, Name, address, charge, total ) VALUES (?, ?, ?, ?, ? ,?)";
        
        $stmt = $db->prepare($query);
        $stmt->bind_param('sssssi', $username, $password , $name, $address, $charge ,$total);
        $stmt->execute();
    ?>
    <p>新增成功！</p>
    ...
    <div class="fixed-bottom">
        ...
        <form action="index.php" method="post" class="column">
            <input type="hidden" name="Username" value="<?=$username?>" />
            <input type="hidden" name="Password" value="<?=$password?>" />
            <input type="submit" name="Submit" id="add" class="bar active" value="✚ 新增" />
        </form>
